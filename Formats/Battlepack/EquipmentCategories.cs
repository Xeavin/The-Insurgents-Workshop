...
{
    public class EquipmentCategories : St2e
    {
        [JsonPropertyName("Equipment Categories")]
        public Dictionary<string, Entry> Entries { get; set; }

        [JsonConstructor]
        public EquipmentCategories(Dictionary<string, Entry> entries)
        {
            Entries = entries;
            SetupHeader((uint)entries.Count, 0x04);
        }

        public EquipmentCategories(string filename)
        {
            ...
            for (var i = 0; i < EntryCount; i++)
            {
                ...
                Entries.Add($"Equipment Category {i}", entry);
            }
        }
