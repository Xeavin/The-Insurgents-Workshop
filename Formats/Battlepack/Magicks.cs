...
{
    public class Magicks : St2e
    {
        [JsonPropertyName("Magicks")]
        public Dictionary<string, Entry> Entries { get; set; }

        [JsonConstructor]
        public Magicks(Dictionary<string, Entry> entries)
        {
            Entries = entries;
            SetupHeader((uint)entries.Count, 0x08);
        }

        public Magicks(string filename)
        {
