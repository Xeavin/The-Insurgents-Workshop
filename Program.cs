...
namespace TheInsurgentsWorkshop
{
    public class Program
    {
        private static void Main()
        {
            ...
            {
                Console.WriteLine("Error: One or more requirements are missing.");
                    ...
                    switch (inputLine)
                    {
                        ...
                        case "3":
                        {
                            ...
                            foreach (var file in OtherFile.FileList)
                            {
                                ...
                                foreach (var path in filePaths)
                                {
                                    var outputPath = Path.ChangeExtension(path.Replace("source", "unpacked"),
                                        file.Extension);
                                    switch (file.Type)
                                    {
                        ...
                        case "4":
                        {
                            ...
                            foreach (var file in Enumerable.Reverse(OtherFile.FileList))
                            {
                                ...
                                foreach (var path in filePaths)
                                {
                                    var inputPath = Path.ChangeExtension(path.Replace("source", "unpacked"),
                                        file.Extension);
                                    var outputPath = path.Replace("source", "newpack");
                                    switch (file.Type)
                                    {
