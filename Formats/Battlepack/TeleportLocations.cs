...
{
    public class TeleportLocations : St2e
    {
        [JsonPropertyName("Teleport Locations")]
        public Dictionary<string, Entry> Entries { get; set; }

        [JsonConstructor]
        public TeleportLocations(Dictionary<string, Entry> entries)
        {
            Entries = entries;
            SetupHeader((uint)entries.Count, 0x0A);
        }

        public TeleportLocations(string filename)
        {
            ...
            for (var i = 0; i < EntryCount; i++)
            {
                ...
                Entries.Add($"Teleport Location {i}", entry);
            }
        }
